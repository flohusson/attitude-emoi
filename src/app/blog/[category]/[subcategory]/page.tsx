...
// Helper to find category from sections
function findCategoryBySlug(slug: string) {
    for (const item of NAVIGATION_ITEMS) {
        if (item.sections) {
            const found = item.sections.find(s => s.slug === slug);
...
export async function generateMetadata({ params }: { params: Promise<{ category: string; subcategory: string }> }) {
    ...
    const expectedHref = `/blog/${categorySlug}/${subCategorySlug}`;
...
export function generateStaticParams() {
    ...
    NAVIGATION_ITEMS.forEach(item => {
        ...
        } else if (item.slug && item.subItems) {
...
export default async function SubCategoryPage({ params }: { params: Promise<{ category: string; subcategory: string }> }) {
    ...
    const breadcrumbSchema = generateBreadcrumbSchema([
        { name: 'Accueil', item: '/' },
        { name: 'Blog', item: '/blog' },
        { name: categoryData.label, item: `/blog/${categorySlug}` },
        { name: subCategoryData.label, item: `/blog/${categorySlug}/${subCategorySlug}` }
    ]);
    ...
    return (
        <main className="min-h-screen pt-12 pb-24">
            ...
            <div className="container mx-auto px-4">
                ...
                <Breadcrumb items={[
                    { name: 'Blog', href: '/blog' },
                    { name: categoryData.label, href: `/blog/${categorySlug}` },
                    { name: subCategoryData.label, href: `/blog/${categorySlug}/${subCategorySlug}` }
                ]} />

                {/* Back Link */}
                <div className="mb-8">
                    <Link href={`/blog/${categorySlug}`} className="inline-flex items-center text-sm font-semibold text-foreground/60 hover:text-primary transition-colors">
