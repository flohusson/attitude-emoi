...
function findCategoryByLabel(label: string) {
    for (const item of NAVIGATION_ITEMS) {
        if (item.sections) {
            const found = item.sections.find(s => s.label === label);
...
export async function generateStaticParams() {
    ...
    for (const article of articles) {
        // Find category from sections
        const categoryItem = findCategoryByLabel(article.category);
...
export default async function ArticlePage({ params }: { params: Promise<{ category: string, subcategory: string, slug: string }> }) {
    ...
    const breadcrumbSchema = generateBreadcrumbSchema([
        { name: 'Accueil', item: '/' },
        { name: 'Blog', item: '/blog' },
        { name: article.category, item: `/blog/${category}` },
        { name: article.subCategory || subcategory, item: `/blog/${category}/${subcategory}` },
    ...
    return (
        <article className="min-h-screen bg-[#FAF9F6]">
            ...
            <div className="max-w-7xl mx-auto px-6 pt-6">
                <Breadcrumb items={[
                    { name: 'Blog', href: '/blog' },
                    { name: article.category, href: `/blog/${category}` },
                    { name: article.subCategory || subcategory, href: `/blog/${category}/${subcategory}` },
